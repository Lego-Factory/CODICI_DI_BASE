...
def homing ():
    ...
    #il pusher controlla la sua posizione cioè controlla se è dentro o fuori
        pusher.run_for_degrees(120,50) #-270° va avanti a velocità 50
        pusher.run_to_position(0,50) #resetto la posizione a velocità 50
    else:
            pusher.run_to_position(0,50) #resetto la posizione a velocità 50
    pusher.run_for_degrees(-120,50)

#muovo il pusher per spostare i pezzi
def pushing ():
     pusher.run_for_degrees(120,30) #-270° va avanti a velocità 50
     pusher.run_to_position(0,30) #resetto la posizione a velocità 50
     pusher.run_for_degrees(-120,30)
    
#facccio andare avanti il conveyor    
def startconveyor():
    conveyor.start(-30) #il motore parte a velocità 50 in direzione negativa
#stop il conveyor    
def stopconveyor():
    conveyor.stop()
